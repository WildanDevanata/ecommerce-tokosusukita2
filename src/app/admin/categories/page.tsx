...
export default function AdminCategoriesPage() {
  ...
  const handleDelete = async (
    ...
    if (
      !confirm(
        'Hapus kategori ini? Semua data terkait mungkin akan terpengaruh.'
      )
    ) {
      return;
    }
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {categories.map((cat) => (
          <div
            key={cat.id}
            className="bg-white rounded-3xl border border-gray-100 p-6 hover:shadow-xl hover:shadow-gray-200/50 transition-all duration-300 group relative overflow-hidden"
          >
            <div className="flex items-start justify-between mb-5">
              <div
                className={`w-14 h-14 ${cat.bgColor} ${cat.color} rounded-2xl flex items-center justify-center text-3xl shadow-inner`}
              >
                {cat.icon}
              </div>
