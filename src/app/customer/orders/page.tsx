...
export default function OrdersPage() {
  ...
  try {
    ...
    // Diarahkan langsung ke API detail order dengan method PATCH
    const res = await fetch(`/api/orders/${orderId}`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ status: 'CANCELLED' }), // Mengirimkan instruksi cancel
    });
            ...
            <div className="flex overflow-x-auto scrollbar-hide">
              {tabs.map((tab) => (
                ...
                >
                  ...
                  {tab.id !== 'ALL' &&
                    myOrders.filter((o: any) => o.status === tab.id).length > 0 && (
                      <span className="ml-2 bg-blue-100 text-blue-600 text-[10px] px-2 py-0.5 rounded-full">
                        {myOrders.filter((o: any) => o.status === tab.id).length}
                      </span>
                    )}
                </button>
              ))}
              ...
              {filtered.map((order: any) => (
                ...
                >
                            ...
                            <img
                              ...
                              onError={(e) => {
                                ...
                                if (e.currentTarget.parentElement) {
                                  e.currentTarget.parentElement.innerHTML = item.productEmoji || '🥛';
                                }
                              }}
                            />
                          ) : (
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 px-6 py-5 border-t border-gray-100">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      {/* Hanya munculkan tombol batal jika status pesanan dan pembayaran masih PENDING */}
