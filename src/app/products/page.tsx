      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {selectedCategories.map(catId => (
              <span key={catId} className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-xs flex items-center gap-2">
                ...
                <X className="w-3 h-3 cursor-pointer" onClick={() => setSelectedCategories(prev => prev.filter(c => c !== catId))} />
              </span>
            ))}
            <button onClick={() => {setSearch(''); setSelectedCategories([]); setMinRating(0);}} className="text-xs text-gray-500 underline">Hapus Filter</button>
          </div>
        )}
        ...
        <div className="flex gap-8">
          ...
          <div className="flex-1">
            {loading ? (
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">Memuat...</div>
            ) : (
              <>
                <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {paginated.map(p => <ProductCard key={p.id} product={p} />)}
                </div>
                {/* Pagination Controls sesuai UI kamu */}
                <div className="mt-8 flex justify-center gap-2">
                  <button onClick={() => setPage(p => Math.max(1, p-1))} className="p-2 border rounded-lg"><ChevronLeft className="w-4 h-4"/></button>
                  <span className="p-2">Halaman {page} dari {totalPages}</span>
                  <button onClick={() => setPage(p => Math.min(totalPages, p+1))} className="p-2 border rounded-lg"><ChevronRight className="w-4 h-4"/></button>
                </div>
              </>
            )}
          </div>
